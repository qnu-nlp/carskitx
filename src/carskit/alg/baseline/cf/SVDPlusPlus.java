...
public class SVDPlusPlus extends BiasedMF {
    ...
    protected double predict(int u, int j, int c) throws Exception {
        ...
        // [VSL] Contexual postfiltering
        
        double pred = predict(u, j);
        
        // Ref: Paper: Yong Zheng, Context-Aware Mobile Recommendation By A Novel Post-Filtering Approach.
		if (isPostFilteringAdjusting && c >= 0) {
			// R(user, item, context) = Beta x R(user, item) + (1 - Beta) x R'(item, context)
			pred = postFilteringAdjusting_Beta * pred + (1 - postFilteringAdjusting_Beta) * getAvgRatingOnContext(j, c);
		} else if (isPostFilteringDeviation && c >= 0) {
			// R(user, item, context) = R(user, item) + Dev(user, item, context)
			double dev = getDevRatingOnContext(u, j, c);
			pred = pred + dev;
		}
