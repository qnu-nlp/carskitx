...
public abstract class Recommender implements Runnable{
    ...
    // [VSL] Contextual postfiltering
    protected boolean isPostFilteringAdjusting=false;
    protected boolean isPostFilteringDeviation = false;
    protected double postFilteringAdjusting_Beta = 0.5;
    protected int postFilteringDev_topKNeighbours = 20;
    protected SymmMatrix userCorrs; // to get K-neighbours
    ...
    protected String getAlgoName()
    {
        ...
        // [VSL] Contextual postfiltering
    	if(isPostFilteringAdjusting)
    		this.algoName += "-PoF-Adj";
    	if(isPostFilteringDeviation)
    		this.algoName += "-PoF-Dev";
    ...
    protected void initModel() throws Exception {
        ...
        // [VSL] build similarity matrix to calculating K-neighbours
        if(isPostFilteringDeviation)
        {
    ...
    public void setPostFilteringAdjusting(double beta) {
    	this.isPostFilteringAdjusting = true;
    	this.postFilteringAdjusting_Beta = beta;
    	this.algoName=this.getAlgoName();
    }
    
    public void setPostFilteringDev(int topK) {
    	this.isPostFilteringDeviation = true;
    	this.postFilteringDev_topKNeighbours = topK;
    	this.algoName=this.getAlgoName();
    }
    
    public double getAvgRatingOnContext(int itemId, int contextId) {
		// Get all ui that the itemId rated.
		Collection<Integer> uiRated = rateDao.getItemRatedList(itemId);

		double sum = 0;
		int count = 0;
		for (int ui : uiRated) {
			SparseVector row = rateMatrix.row(ui);
			if (row.contains(contextId)) {
				double rate = row.get(contextId);
	...
	public double getDevRatingOnContext(int userId, int itemId, int contextId) throws Exception {
		double dev = 0;

		Map<Integer, Double> kNeighbours = getKNeighbours(userId, postFilteringDev_topKNeighbours, itemId);
		double a = 0, b = 0;

		for (Map.Entry<Integer, Double> en : kNeighbours.entrySet()) {
			// en.getKey(): User Id; en.getValue(): Similarity
			double ratingNoContext = getRating(en.getKey(), itemId);
			double ratingInContext = getRating(en.getKey(), itemId, contextId);
			if (ratingInContext > 0 && ratingNoContext > 0) {
				a += (ratingInContext - ratingNoContext) * en.getValue();
				b += en.getValue();
			}
		}
	...
	// Get K-neighbours of user u who rated itemId.
	public Map<Integer, Double> getKNeighbours(int u, int topK, int itemId) throws Exception {
		// find a number of similar users
		Map<Integer, Double> nns = new HashMap<>();
		if(topK <= 0) return nns;
		...
		for (int v : dv.getIndex()) {
			...
			if (sim <= 0)
				continue;
			if (itemId >= 0) {
				double rate = train.get(v, itemId);
				if (rate > 0) // means the itemId rated
					nns.put(v, sim);
			} else
				nns.put(v, sim);
		}
		...
		if (topK < nns.size()) {
			...
			List<Map.Entry<Integer, Double>> subset = sorted.subList(0, topK);
			nns.clear();
	...
	// Get rating (ignore contextual information)
	public double getRating(int userId, int itemId) {
		return train.get(userId, itemId);
	}

	// Get rating in specific context
	public double getRating(int userId, int itemId, int contextId) {
		int uiId = rateDao.getUserItemId(userId + "," + itemId);
		return this.trainMatrix.get(uiId, contextId);
	}

    
    ////////////// [VSL] End of Contextual postfiltering /////////////////////////////////////////
